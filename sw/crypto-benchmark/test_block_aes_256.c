...
void test_aes_256(int num_tests) {
    ...
    for(int i = 0; i < num_tests; i++)
    {
    	for(int j = 0; j < AES_256_RK_WORDS; j++)
    	{
    		erk[j] = 0;
    		drk[j] = 0;
    	}
    	...
    	if(error_flag == 0){
			...
			printf("test failed!\n");
	    	printf("key = ");puthex(key, AES_256_KEY_BYTES);printf("\n");
	    	printf("erk = ");puthex((uint8_t *)erk, AES_256_RK_WORDS);printf("\n");
	    	printf("drk = ");puthex((uint8_t *)drk, AES_256_RK_WORDS);printf("\n");
