...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < TEST_COUNT; i++) {
    ...
    #elif defined TEST_TIMES
    const uint64_t end_time = __get_rv_time();
    #endif
    ...
    #elif defined TEST_TIMES
    const uint64_t final_time = end_time - start_time;
    #endif
    ...
    printf("ipb             = 0 if input_len == 0 else instr_count / "
           "input_len\n");
    #elif defined TEST_CYCLES
    printf("bpc             = input_len / cycle_count\n");
    #endif
    ...
    #if defined TEST_INSTRS
    printf(
        "    print(\"" STR(SM3) " Test %%d passed. "
                                      "%%d instrs / %%d bytes. IPB=%%f\" %% "
                                      "(testnum,instr_count,input_len,ipb))\n");
    #elif defined TEST_CYCLES
    printf(
        "    print(\"" STR(SM3) " Test %%d passed. "
                                      "%%d cycles / %%d bytes. BPC=%%f\" %% "
                                      "(testnum,cycle_count,input_len,bpc))\n");
    #elif defined TEST_TIMES
    printf(
        "    print(\"" STR(SM3) " Test %%d passed. \n");
    #endif
...
#else
int main(int argc, char **argv){
    printf("This is a test for SM3 module \n");
    size_t message_lengths = 3;
    uint8_t *messages = {(uint8_t *)"abc"};
    uint8_t expected_digests[32] = {0x66, 0xC7, 0xF0, 0xF4, 0x62, 0xEE, 0xED, 0xD9, 0xD1, 0xF2, 0xD4,
                                    0x6B, 0xDC, 0x10, 0xE4, 0xE2, 0x41, 0x67, 0xC4, 0x87, 0x5C, 0xF2,
                                    0xF7, 0xA2, 0x29, 0x7D, 0xA0, 0x2B, 0x8F, 0x4B, 0xA8, 0xE0};
    uint8_t actual_digest[32];

    sm3_hash(actual_digest, messages, message_lengths);
    
    printf("messages:");
    for(int i = 0; i < message_lengths; i++)
        printf("%02x",messages[i]);
    printf("\n");
    printf("hash value: \n");
    for(int i = 0; i < 32; i++)
        printf("%02x",actual_digest[i]);
    printf("\n");
}
#endif